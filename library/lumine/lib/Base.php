...
class Lumine_Base extends Lumine_EventListener implements Iterator
{
	...
	function __construct( $data = null )
	{
		...
		$this->_getConfiguration()->dispatchEvent(
			new Lumine_ConfigurationEvent(Lumine_Event::CREATE_OBJECT, $this->_getConfiguration(), $this)
		);
	...
	public function destroy()
	{
		...
		if(!empty($this->_foreign)){
			foreach($this->_foreign as $key => $name){
				if(!empty($this->$key)){
	...
	{
		$this->dispatchEvent(new Lumine_SQLEvent(Lumine_Event::PRE_GET, $this));
		...
		if( !empty($pk) && ! empty($pkValue) )
		{
			...
			if( empty($list) )
			{
				Lumine_Log::warning( 'A entidade '.$this->_getName().'  possui chave prim�ria. Especifique um campo.');
				return 0;
			}
			...
			$this->dispatchEvent(new Lumine_SQLEvent(Lumine_Event::POS_GET, $this));
			return $this->_getDialect()->num_rows();
		}
		Lumine_Log::warning('Nenhum valor informado para recuperacao em '.$this->_getName());
	}
	...
	{
		$this->dispatchEvent(new Lumine_SQLEvent(Lumine_Event::PRE_FIND, $this));
		...
		$this->dispatchEvent(new Lumine_SQLEvent(Lumine_Event::POS_FIND, $this));
	...
	{
		$this->dispatchEvent(new Lumine_IteratorEvent(Lumine_Event::PRE_FETCH, $this));
		...
		$this->dispatchEvent(new Lumine_IteratorEvent(Lumine_Event::POS_FETCH, $this));
	...
	public function join( Lumine_Base $obj, $type = 'INNER', $alias = '', $linkName = null, $linkTo = null, $extraCondition = null )
	{
		...
		if( $opt != null && $dest != null )
		{
			...
			if( $opt['type'] == self::MANY_TO_MANY && $dest['type'] == self::MANY_TO_MANY && $opt['table'] == $dest['table'] )
			{
				...
				$this_link = $this->_getField( $opt['linkOn'] );			// pega o campo referente a uniao desta entidade
				$dest_link = $obj->_getField( $dest['linkOn'] );			// pega o campo referente a uniao da entidade que esta sendo unida
				...
				$joinString = sprintf( $joinString,							// faz o parse colocando...
					...
					$schema . $obj->tablename(),							// ... o nome da tabela que esta sendo unida
	...
	public function joinExpression($obj, $type, $expression, $alias = null){
		...
		$this->_join[] = sprintf('%s JOIN %s %s ON %s', $type, $obj->tablename(), $alias, $expression);
	...
	{
		$this->dispatchEvent(new Lumine_SQLEvent(Lumine_Event::PRE_INSERT, $this));
		...
		if($result == true)
		{
			...
			$this->dispatchEvent(new Lumine_SQLEvent(Lumine_Event::POS_INSERT, $this, $sql));
		}
	...
	{
		$this->dispatchEvent(new Lumine_SQLEvent(Lumine_Event::PRE_SAVE, $this));
		...
		$this->dispatchEvent(new Lumine_SQLEvent(Lumine_Event::POS_SAVE, $this));
	...
	{
		$this->dispatchEvent(new Lumine_SQLEvent(Lumine_Event::PRE_UPDATE, $this));
		...
		if( $sql !== false )
		{
			...
			$this->dispatchEvent(new Lumine_SQLEvent(Lumine_Event::POS_UPDATE, $this, $sql));
			$this->saveDependentObjects();
	...
	{
		$this->dispatchEvent(new Lumine_SQLEvent(Lumine_Event::PRE_DELETE, $this));
		...
		$this->dispatchEvent(new Lumine_SQLEvent(Lumine_Event::POS_DELETE, $this, $sql));
	...
	public function _getName()
	{
     ...
     /**
      * Recupera a definicao dos campos da classe
      * Especifico para as colunas da tabela representada pela classe
      * @author Hugo Ferreira da Silva
      * @link http://www.hufersil.com.br/lumine
      * @return array Lista contendo as definicoes de cada campo
      */
	public function _getDefinition()
	{
      ...
      * @author Hugo Ferreira da Silva
      * @link http://www.hufersil.com.br/lumine
      * @return array Lista contendo as chaves estrangeiras e suas definicoes
      */
	public function _getForeignRelations()
	{
      ...
      * @author Hugo Ferreira da Silva
      * @link http://www.hufersil.com.br/lumine
      * @return array Array associativo contendo a definicao do campo
	...
	public function _getField( $name )
	{
		...
		throw new Lumine_Exception('O campo '.$name.' nao foi encontrado em '.$this->_getName(), Lumine_Exception::ERROR);
	}
	...
	public function _getFieldByColumn( $column )
	{
      ...
      * Depois de alterado, voce podera usar o alias seguido do nome do membro da classe
      * <code>
	  * $pes = new Pessoa;
	  * $pes->_setAlias('p');
	 ...
	 * @see Lumine_Base::_setAlias()
	 * @see Lumine_Base::_getAlias()
	 */
	...
	public function _getObjectPart($partName)
	{
     ...
     /**
      * Altera a configuracao atual do objeto.
      *
      * @param Lumine_Configuration $config Novo objeto de configuracao
      * @author Hugo Ferreira da Silva
      * @link http://www.hufersil.com.br/lumine
      * @deprecated
      */
	...
	public function _getConfiguration()
	{
      ...
      *
      * @param ILumine_Connection $cn Novo objeto de conexao
      * @author Hugo Ferreira da Silva
      * @link http://www.hufersil.com.br/lumine
      * @return void
      */
	public function _setConnection(ILumine_Connection $cn )
	{
      ...
      * @author Hugo Ferreira da Silva
      * @link http://www.hufersil.com.br/lumine
      * @return ILumine_Connection Objeto de conexao com o banco atual
      */
	public function _getConnection()
	{
     ...
     /**
      * Recupera uma lista contendo as chaves primarias
      *
      * @author Hugo Ferreira da Silva
      * @link http://www.hufersil.com.br/lumine
      * @return array Lista das chaves primarias
      */
		...
		try
		{
			...
			switch($field['type'])
			{
				...
				case self::MANY_TO_MANY:
					...
					$fieldlink   = $list->_getField( $campoEstrangeiro['linkOn'] );			// pega a definicao do campo estrangeiro
					$reffield    = $this->_getField( $field['linkOn'] );					// pega a definicao do campo desta entidade
					...
					$tabelaLink  = $list->tablename();										// pega o nome da entidade que sera linkada
		...
		try {
			...
			switch($field['type']){
				...
				case self::MANY_TO_MANY:
					...
					$rel = $obj->_getRelation(get_class($this));
					$field_obj = $obj->_getField($rel['linkOn']);
	...
	{
		Lumine_Log::debug( "Por questoes de compatibilidade, use _setFrom");
		return $this->_setFrom($arr);
	}
	
    /**
     * @see Lumine_Base::_getLink()
	 * @deprecated
     */
	public function getLink($linkName)
	{
		Lumine_Log::debug( "Por questoes de compatibilidade, use _getLink");
	...
	{
		try {
			$this->dispatchEvent( new Lumine_SQLEvent(Lumine_Event::PRE_QUERY, $this, $sql) );
			$rs = $this->_execute( $sql );
			$this->dispatchEvent( new Lumine_SQLEvent(Lumine_Event::POS_QUERY, $this, $sql) );
	...
	public function multiInsert( $ignoreAutoIncrement = true )
	{
		...
		$this->dispatchEvent(new Lumine_SQLEvent(Lumine_Event::PRE_MULTI_INSERT, $this, $sql));									// dispara o pre-evento
		$this->_execute( $sql );																// executa a insercao
		$this->dispatchEvent(new Lumine_SQLEvent(Lumine_Event::POS_MULTI_INSERT, $this, $sql));									// dispara o pos evento
	...
	{
		$this->_setFieldOption($name, $option, $value);
		return $this;
	}
	...
	{
		return $this->_getFieldOption($name, $option);
	}
	 ...
	 * @return Lumine_Base
	 */
	public function _setFieldOption($name, $option, $value)
	{
		foreach($this->_definition as $fldname => $def){
			if($fldname == $name) {
				$this->_definition[$fldname]['options'][$option] = $value;
			}
		}
		
		foreach($this->_foreign as $fldname => $def){
			if($fldname == $name) {
				$this->_foreign[$fldname][$option] = $value;
			}
		}
	 ...
	 * @return mixed Valor da opcao
	 */
	public function _getFieldOption($name, $option)
	{
		try {
			$fld = $this->_getField($name);
	...
	protected function _getMultiInsertSQL($ignoreAutoIncrement = true)
	{
		...
		reset( $this->_definition );															// reinicia a definicao
		
		foreach( $this->_definition as $name => $prop )											// para cada item da definicao
		{
     ...
     /**
      * Adiciona um campo a classe para fazer relacionamento a coluna da tabela
      *
	  ...
	  * @param array $options Opcoes do campo
      * @author Hugo Ferreira da Silva
      * @link http://www.hufersil.com.br/lumine
      * @return void
      */	
	protected function _addField($name, $column, $type, $length, array $options)
	{
		...
		// checamos se a opcao de formatadores
		// foi adicionada
		if(!empty($options['formatter'])){
			$this->addFormatter($name, $options['formatter']);
		}
	}
	  ...
	  * @param string $class Nome da classe que sera relacionada
	  * @param string $linkOn Nome do campo da entidade que sera referenciada
	  * @param string $table Nome da tabela de relacionamentos many-to-many
	  * @param string $column Nome da coluna da tabela mtm que referencia a chave primaria da classe atual
	...
	protected function _addForeignRelation($name, $type, $class, $linkOn, $table = null, $column = null, $lazy = false)
	{
      ...
      *
      * @param string $entityName Nome da entidade que sera recuperada
      * @param int $type Tipo de relacionamento
      * @author Hugo Ferreira da Silva
      * @link http://www.hufersil.com.br/lumine
      * @return array Definicao do relacionamento
      */	
	protected function _getRelation( $entityName, $type = NULL )
	{
		...
		{
			if( $options['class'] == $entityName && (is_null($type) || $type == $options['type']) )
			{
				$opt = $options;
				$opt['name'] = $name;
				return $opt;
			...
			if( !empty($prop['options']['primary']) && !empty($prop['options']['foreign']) && !empty($prop['options']['class']) )
			{
				...
				if( strtolower($class) == strtolower($prop['options']['class']) )
				{
					...
					if( !is_null( $chave ) )
					{
						...
						Lumine_Log::debug('Valor nao encontrado para: "' . $obj->_getName(). '" com o nome de campo ' . $name . '-> '.$chave);
					...
					if( !empty($list) && is_array($list))
					{
						foreach($list as $val)
						{
							...
							if($val instanceof Lumine_Base)
							{
								...
								try {
									$field = $val->_getRelation($relname, self::MANY_TO_ONE);
									$val->setFieldValue($field['name'], $this->$field['linkOn']);
									$val->save();
									
								} catch (Lumine_Exception $e) {
									Lumine_log::warning('nao foi possivel encontrar o campo '.$relname.' em '.$val->_getName());
								}
							}
						}
					}
					...
					if( !empty($list) && is_array($list))
					{
						foreach($list as $val)
						{
							...
							if($val instanceof Lumine_Base)
							{
								...
								$rel = $val->_getRelation( $this->_getName(), self::MANY_TO_MANY );
								...
								if( !is_null($valor_pk))
								{
									...
									Lumine_Log::warning('A o campo "'.$pks[0]['name'].' da classe "'.$this->_getName().'" nao possui um valor');
								}
							}
						}
					}
	...
	protected function loadLazy( $except = null )
	{
		...
		foreach($this->_definition as $name => $def)
		{
			...
			{
				$this->setFieldValue($name, $this->_getLink( $name ));
			}
		}
	...
	private function _checkMemberExistence( $memberName )
	{
		...
		{
			$this->_classDefinition = get_class_vars($this->_getName());
		}
	...
	private function _checkMethodExistence($methodName)
	{
		...
		{
			$this->_classMethods = get_class_methods($this->_getName());
		}
	...
	private function _cleanFields(){
		...
		foreach( $this->_definition as $name => $dummy )
		{
			$this->$name = null;
		}
		foreach( $this->_foreign as $name => $dummy )
		{
			$this->$name = null;
		}
	}
	...
	{
		try {
			$this->dispatchEvent(new Lumine_SQLEvent(Lumine_Event::PRE_EXECUTE, $this, $sql));
			$this->connect();
			$result = $this->_getDialect()->execute($sql);
			$this->dispatchEvent(new Lumine_SQLEvent(Lumine_Event::POS_EXECUTE, $this, $sql));
	...
	protected function _getFromParent( $key )
	{
		try {
			...
			if( is_null($super) )
			{
				throw new Exception('Super-Classe nao encontrada para ' . $this->_getName());
			}
