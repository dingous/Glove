...
class _tree_struct {
	...
	// Constructor
	function __construct($table = "tb_gcool_categoria", $fields = array()) {
		$this->table = $table;
		if(!count($fields)) {
		...
		// Database
		if(extension_loaded("mysqli")) $this->db = new My_Plugins_jstree_databasei();
		else $this->db = new My_Plugins_jstree_database();
	}
	...
	function _get_children($id, $recursive = false) {
		...
		if($recursive) {
			...
			if($id == 1)
			{
				$sql = "SELECT `id` , `parent_id` , `position` , `left` , `right` , `level` , `title` , `type` FROM `tb_gcool_categoria` WHERE `parent_id` = 1 ORDER BY `position` ASC";
			}else {
				$sql = "SELECT `id` , `parent_id` , `position` , `left` ,`right` , `level` , CONCAT(`title`, ' (', COUNT(*) , ')') as `title` , `type`  FROM  tb_gcool_local inner join `tb_gcool_categoria` on tb_gcool_categoria.id = tb_gcool_local.ID_CATEGORIA WHERE tb_gcool_local.ID_CIDADE = $franquia->ID_CIDADE AND `parent_id` = ".(int) $id." group by id ORDER BY `position` ASC";
			}
			$this->db->query($sql);
...
class My_Plugins_jstree_classtreeCategoriasdoPortal extends _tree_struct { 
	function __construct($table = "tb_gcool_categoria", $fields = array(), $add_fields = array("title" => "title", "type" => "type")) {
