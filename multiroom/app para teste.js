...
app.get('/chat', function (req, res) {
	...
	var sql = "SELECT tb_sys_usuario.ID_USUARIO, tb_sys_usuario.NOME, tb_sys_usuario.SLUG, tb_glove_foto.ARQUIVO ";
		sql += "FROM tb_glove_amigos ";
		sql += "INNER JOIN tb_sys_usuario ON tb_glove_amigos.ID_USUARIO = tb_sys_usuario.ID_USUARIO ";
		sql += "INNER JOIN tb_glove_perfil ON tb_sys_usuario.ID_USUARIO = tb_glove_perfil.ID_USUARIO ";
		sql += "LEFT JOIN tb_glove_foto ON tb_glove_perfil.ID_FOTOPERFIL = tb_glove_foto.ID_FOTO ";
		sql += "WHERE tb_glove_amigos.ID_AMIGO = '23' ";
		sql += "AND FL_ACEITOU = 1 ";
		sql += "ORDER BY tb_glove_amigos.DT_CADASTRO DESC";
	client.query(sql, 
...
var chat = io		 
		  ...
		  .on('connection', function (socket) {
			...
			socket.on('registerlogChat', function (data) 
			{
				...
				if (dia < 10){ dia = "0" + dia}
				if (mes < 10){ mes = "0" + (mes + 1) }else{ mes = mes + 1}
				if (ano < 2000){ ano = "19" + ano }
				var data = ano+"/"+mes+"/"+dia;
				
				var sql = "SELECT * FROM tb_glove_chat AS MSG WHERE 1=1 AND MSG.ID_USUARIO_REMETENTE = '"+remetente+"' AND MSG.ID_USUARIO_DESTINATARIO = '"+destinatario+"' AND DATE_FORMAT(MSG.DATA_CADASTRO, '%Y/%m/%d') = '"+data+"'";
				
				//console.log(sql);
				
				var sql2 = "INSERT INTO  `tb_glove_chat` (";
					sql2 += "`ID_CHAT` ,";
				...
				function(err, results, fields) {
					...
					{
						/*var sql3 = "UPDATE  `tb_glove_chat` SET  `MENSAGEM` =  '"+msg+"', DATA_MODIFICACAO = '"+data+"' WHERE `tb_glove_chat`.`ID_CHAT` = '"+results[0].ID_CHAT+"';";*/
						
						var sql3 = "UPDATE  `tb_glove_chat` SET  `MENSAGEM` =  '"+msg+"', DATA_MODIFICACAO = '"+data+"' WHERE 1=1 AND ID_USUARIO_REMETENTE = '"+remetente+"' AND ID_USUARIO_DESTINATARIO = '"+destinatario+"' AND DATE_FORMAT(DATA_CADASTRO, '%Y/%m/%d') = '"+data+"'";
						
						var sql4 = "UPDATE  `tb_glove_chat` SET  `MENSAGEM` =  '"+msg+"', DATA_MODIFICACAO = '"+data+"' WHERE 1=1 AND ID_USUARIO_REMETENTE = '"+destinatario+"' AND ID_USUARIO_DESTINATARIO = '"+remetente+"' AND DATE_FORMAT(DATA_CADASTRO, '%Y/%m/%d') = '"+data+"'";
						
						
						client.query(sql3, function(err, results, fields) { });
						client.query(sql4, function(err, results, fields) { });						
